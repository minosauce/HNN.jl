module LuxHNN
    ...
    @inline function hamiltonian(hnn::HamiltonianNN, x::AbstractVector, ps, st::NamedTuple)
        ...
        H, _ = Lux.apply(hnn.model, x, ps, st)
        return only(H)
    end
    ...
    function grad_HNN(hnn::HamiltonianNN, X::AbstractMatrix, ps, st::NamedTuple)
        f(u) = sum(hamiltonian(hnn, u, ps, st))
        ∇H = DI.gradient(f, hnn.ad, X)   # (n × B)
        return ∇H
    end

    # 3. ẋ = J ∇H(x) (call overloading)
    # vector
    function (hnn::HamiltonianNN)(x::AbstractVecOrMat, ps, st::NamedTuple)
